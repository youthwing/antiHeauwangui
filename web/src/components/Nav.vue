...
<template>
  <header class="sticky top-0 z-40 bg-white/70 dark:bg-[#0d1117]/70 backdrop-blur-xl border-b border-black/[0.08] dark:border-white/[0.06]">
    <div class="max-w-3xl mx-auto px-4 h-14 flex justify-between items-center">
      ...
      <div ref="menuRef" class="relative">
        ...
        >
          <span class="w-7 h-7 rounded-md bg-red-500/15 ring-1 ring-red-500/30 text-red-400 text-xs font-semibold flex items-center justify-center">
            {{ name?.[0] || '?' }}
          </span>
          <span class="text-sm text-[#161b22] dark:text-zinc-200">{{ name }}</span>
          <ChevronDown
        ...
        <Transition name="menu">
          <div
            v-if="menuOpen"
            class="absolute right-0 top-full mt-2 w-44 rounded-xl bg-zinc-100 dark:bg-[#161b22] ring-1 ring-black/10 dark:ring-white/10 shadow-2xl overflow-hidden"
          >
            <button
