...
// precheck → 'token' (JWT first, wechat callback as fallback).
...
async function submit() {
  ...
  // Activate flow, step 1: precheck the invite code before revealing any
  // school-token input.
...
async function toggleWechatQr() {
  showWechatQr.value = !showWechatQr.value
  if (showWechatQr.value && !wechatQrDataUrl.value && !buildingWechatQr.value) {
    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-white/85 dark:bg-[#161b22]/60 ring-1 ring-black/[0.08] dark:ring-white/[0.06] rounded-2xl backdrop-blur-sm overflow-hidden">
        ...
        <div v-else class="p-6">
          <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-5">首次使用，用邀请码 + 学校 JWT 完成激活，并设置登录 PIN</p>
          ...
          <div class="space-y-4">
            ...
            <div v-show="activateStep === 'token'" class="space-y-4">
              ...
              <div class="rounded-2xl bg-red-500/[0.08] ring-1 ring-red-500/30 p-4 shadow-[0_10px_30px_rgba(16,185,129,0.08)]">
                ...
                <p class="mt-1 text-[11px] text-zinc-600 dark:text-zinc-400 leading-relaxed">
                  复制 Authorization / accessToken 里的完整 JWT，通常是
                  <code class="bg-white/80 dark:bg-[#161b22]/80 px-1 rounded text-zinc-700 dark:text-zinc-300">eyJ...</code>
                  开头的三段字符串。
                </p>
                <textarea
                  v-model="token"
                  placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
                  class="mt-3 w-full bg-white dark:bg-[#0d1117] ring-2 ring-red-500/25 focus:!ring-red-500/55 rounded-xl px-3 py-3 h-32 resize-none text-[#161b22] dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus-ring font-mono-token"
                />
              </div>
              ...
              <div class="rounded-xl bg-white/70 dark:bg-[#0d1117]/70 ring-1 ring-black/[0.05] dark:ring-white/[0.04] p-3">
                <button
                  ...
                  class="w-full flex items-center justify-between gap-3 text-left text-xs text-zinc-600 dark:text-zinc-400 hover:text-[#161b22] dark:hover:text-zinc-200 transition-colors"
                >
                  <span class="inline-flex items-center gap-1.5">
                    <QrCode class="w-3.5 h-3.5" />
                    没有 JWT？使用微信扫码回调
                  </span>
                  <span class="text-[10px] text-zinc-500 dark:text-zinc-600">{{ showWechatQr ? '收起 ↑' : '展开 ↓' }}</span>
                </button>

                <Transition name="expand">
                  <div v-if="showWechatQr" class="mt-3 overflow-hidden">
                    <div class="flex flex-col sm:flex-row gap-4">
                      ...
                      <div class="min-w-0 flex-1">
                        <ol class="text-[12px] text-zinc-700 dark:text-zinc-300 space-y-2 list-decimal list-inside leading-relaxed">
                          <li>用手机<strong>微信</strong>扫码</li>
                          <li>跳到学校晚归页面后正常登录</li>
                          <li>登录成功后复制回调链接或 code</li>
                          <li>粘到下面输入框</li>
                        </ol>
                        <button
                          type="button"
                          @click="refreshWechatQr"
                          class="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] text-zinc-700 dark:text-zinc-300 bg-white/80 dark:bg-[#161b22]/80 ring-1 ring-black/[0.06] dark:ring-white/[0.05] hover:ring-red-500/40 transition-colors"
                        >
                    ...
                    <textarea
                      ...
                      class="mt-3 w-full bg-white dark:bg-[#0d1117] ring-1 ring-black/[0.08] dark:ring-white/[0.06] rounded-xl px-3 py-3 h-28 resize-none text-[#161b22] dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus-ring"
                    />
                    <div
...
<style scoped>
.expand-enter-active, .expand-leave-active { transition: all 0.25s ease; max-height: 720px; }
