...
function doExport() {
  const url = `/api/v1/airvel/records.csv?from=${exportFrom.value}&to=${exportTo.value}`
...
const meta: Record<string, { color: string; dotBg: string; label: string }> = {
  success: { color: 'text-red-400', dotBg: 'bg-red-500', label: '成功' },
  already: { color: 'text-blue-400', dotBg: 'bg-sky-500', label: '已签' },
...
<template>
  <div class="space-y-3">
    ...
    <section class="rounded-xl bg-white/85 dark:bg-[#161b22]/60 ring-1 ring-black/[0.08] dark:ring-white/[0.06] p-4 flex flex-col sm:flex-row sm:items-end gap-3">
      <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold text-[#161b22] dark:text-zinc-200 mb-0.5">导出 CSV</p>
      ...
      <div class="flex items-end gap-2">
        <div>
          ...
          <input type="date" v-model="exportFrom"
            class="bg-white dark:bg-[#0d1117] ring-1 ring-black/[0.08] dark:ring-white/[0.06] rounded-md px-2 py-1.5 text-xs font-mono-token focus-ring text-[#161b22] dark:text-zinc-200" />
        </div>
        <div>
        ...
        <button
          @click="doExport"
          class="inline-flex items-center gap-1.5 bg-red-500 hover:bg-red-400 text-[#0d1117] text-xs font-medium px-3 py-1.5 rounded-md transition-colors"
        >
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <RouterLink :to="`/airvel/users`" class="text-sm text-zinc-700 dark:text-zinc-300 hover:text-red-400 transition-colors">
                  {{ r.userName || r.userId }}
                </RouterLink>
